...
public class MyShiroRealm extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principalCollection) {
        ...
        if (StringUtils.isNotEmpty(shiroUser.getId())) {
            try {
                ...
                List<Map> roleList = sysUserRoleService.roleListByUserId(Integer.parseInt(shiroUser.getId()));
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken) throws AuthenticationException {
        ...
        SysUserEntity user = sysUserService.getUserByAccount(account);

        if (user == null) {
            throw new UnknownAccountException();
        } else {
            if (UserStatusEnum.DISABLE.getNodeCode().equals(user.getStatus())) {
        ...
        try {
            //查询用户的角色
            List<SysUserRoleEntity> roleList = sysUserRoleService.roleListByUserId(user.getId());
            List<Map<String, Object>> listResource = new ArrayList<>();

            for (int i = 0; i < roleList.size(); i++) {
                List permissionList = sysRolePermissionService.permissionList(roleList.get(i).getRoleid().toString());
            ...
            if (UserStatusEnum.NOTACTIVE.getNodeCode().equals(user.getStatus())) {
                SysUserEntity sysUserEntity = new SysUserEntity();
                sysUserEntity.setStatus(UserStatusEnum.ACTIVATE.getNodeCode());
                sysUserEntity.setUpdatetime(new Date());
                sysUserEntity.setId(user.getId());

                sysUserService.loginUpdateUser(sysUserEntity);
        ...
        ShiroUser shiroUser = new ShiroUser(user.getId().toString(),
                user.getLoginname(), user.getUsername());
        return new SimpleAuthenticationInfo(shiroUser, user.getPassword(), salt, getName());
    }
